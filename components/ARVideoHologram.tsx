...
const ARVideoHologram = ({ messageId, arMessage, onVideoUpload, onGestureVerified }: ARVideoHologramProps) => {
    ...
    // Define stopStream and cleanupUnity before useEffect
    ...
    useEffect(() => {
        ...
        window.onUnityARVideoReady = () => {
            console.log("Unity signaled readiness via onUnityARVideoReady");
        ...
        window.onUnityVideoStarted = () => {
            console.log("Unity video started playing");
            setIsPlaying(true);
        };

        window.onUnityVideoStopped = () => {
            console.log("Unity video stopped playing");
            setIsPlaying(false);
        };
    ...
    }, [stopStream, cachedVideoUrl, cleanupUnity]);
    ...
    const loadUnity = useCallback(async () => {
        ...
        try {
            ...
            setUnityDebugInfo((prev) => [...prev, "Canvas created and added to container"]);
            ...
            script.onload = () => {
                ...
                if (!createUnityInstance) throw new Error("Unity createUnityInstance not found");
                ...
                createUnityInstance(canvas, {
                    ...
                    .catch((error: any) => {
                        ...
                        toast.error("Failed to load Unity: " + error.message);
                        setIsUnityLoading(false);
    ...
    }, [unityInstance, isUnityLoaded, isUnityLoading, unityLoadAttempts, patchWebGLContext]);

    const startFallbackVideoPlayer = useCallback(async () => {
        const videoUrl = await loadCachedVideo() || "https://www.w3schools.com/html/mov_bbb.mp4";
        console.log("Fallback video URL:", videoUrl);
        if (!videoUrl) {
            toast.error("Failed to load video URL for fallback");
            return;
        }
        ...
        if (unityContainerRef.current) {
            ...
            videoElement.onerror = () => console.error("Fallback video failed to load");
    ...
    const startUnityDisplay = useCallback(async () => {
        console.log("startUnityDisplay called");

        if (!isUnityLoaded && !isUnityLoading) {
            console.log("Unity not loaded yet, initiating load...");
            loadUnity();
        ...
        if (!unityInstance) {
            console.log("No Unity instance available yet");
            if (unityLoadAttempts >= 3) {
                console.log("Max Unity load attempts reached, switching to fallback");
                toast.error("Unity failed to load after multiple attempts. Using fallback player.");
                startFallbackVideoPlayer();
                return;
            }
            if (!isUnityLoading) {
                console.log("Retrying Unity load...");
                loadUnity();
        ...
        console.log("Unity instance exists, proceeding with video playback...");
        toast.loading("Preparing video playback...", { id: "video-loading" });
        ...
        try {
            ...
            console.log("Sending SendMessage to Unity with URL:", videoUrl);
            unityInstance.SendMessage("ARVideoPlayer", "LoadAndPlayVideo", videoUrl);
            console.log("SendMessage executed successfully");

            if (unityContainerRef.current) {
                unityContainerRef.current.style.display = "block";
                console.log("Unity container set to visible");
            }

            setIsPlaying(true);
            toast.success("Video playback command sent to Unity!");
            toast.dismiss("video-loading");
        } catch (error) {
            console.error("Error in startUnityDisplay:", error);
    ...
    const stopUnityDisplay = useCallback(() => {
        if (unityInstance) {
            try {
                unityInstance.SendMessage("ARVideoPlayer", "ResetVideoPlacement");
                setIsPlaying(false);
                toast.success("Unity video display stopped!");
            } catch (error) {
                console.error("Error stopping Unity display:", error);
                toast.error("Failed to stop Unity video display");
            }
        } else {
            if (unityContainerRef.current) {
                unityContainerRef.current.innerHTML = "";
                setIsPlaying(false);
                toast.success("Video display stopped!");
            }
        }
    }, [unityInstance]);
    ...
    return (
        <div className="space-y-4">
            <div
                ...
                id="unityContainer"
                className="w-full h-[50vh] bg-black rounded-lg overflow-hidden"
                    ...
                    <div className="mt-3">
                        {!arMessage.isViewed && !gestureVerified ? (
                            <div className="flex flex-col items-start space-y-2">
                                ...
                                >
                                    <Video className="w-4 h-4 mr-2" />
                                    Reveal Video ({arMessage.gestureTrigger})
                                </Button>
                            ...
                            <div className="flex flex-col items-start space-y-2">
                                ...
                                >
                                    <Video className="w-4 h-4 mr-2" />
                                    View Video {isUnityLoading && "(Loading...)"}
                                </Button>
                                <span className="text-xs text-gray-500">
                                    {isUnityLoaded
                                        ? "Gesture verified, click to start Unity video display"
                                        : isUnityLoading
                                            ? "Loading Unity environment, please wait..."
                                            : "Click to load Unity video display"}
                                </span>
                            </div>
                        ) : isPlaying ? (
                            ...
                            <span className="text-sm text-gray-500">Video Viewed</span>
                        )}
